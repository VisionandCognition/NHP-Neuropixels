...
**Why some visually-peaked-looking curves in `tuning_examples.png` aren't classified as tuned.** With ~8-18 trials per direction and single-channel MUA noise, this is a mix of two things, not just one: (1) the figure itself, until now, min-max normalized every curve to its own [0,1] range with no error bars — this stretches any wobble, real or pure noise, to fill the same visual range, so a small noisy peak looked identical to a large reliable one; (2) genuinely borderline cases are common at this trial count. Checked directly: channel 269 (20260323) has a clean-looking peak (normalized 1.0 at 315°, next-highest 0.29) that remains visually convincing even WITH SEM error bars added (peak's lower bound clears every other direction's upper bound) — yet `pPerm` ≈ 0.055-0.061 across repeat runs, just missing the p<0.05 cutoff. This is not hidden noise or an overly strict threshold; it is a real value sitting right at the significance boundary, which is expected to happen for *some* channels given how significance testing works, not a sign of miscalibration. `tuning_examples.png` now plots SEM error bars (scaled by the same per-channel normalization factor as the curve itself) specifically so this distinction — "a visually strong peak with tight error bars" vs. "a visually strong peak whose error bars overlap the rest" — is visible directly in the figure, rather than requiring a trip to `tuning_results.mat`. The same fix applies to the `*_planexec_<mode>_curves.png` figures (§4.8) — `analyzePlanningExecution.m`'s `fitTuning` now also computes `semByDir`, plotted as error bars in raw (non-normalized) response units there.
...
- **`*_tuning_examples.png`** — all responsive channels, sorted by depth, one small tuning-curve tile per channel (min-max normalized per channel for display, with SEM error bars scaled by the same normalization factor — see §4.7), border and line color reflecting the 3-way category from §4.7: red=`directional`, orange=`complex`, gray=`untuned`.
...
- **`*_planexec_gocue_curves.png` / `*_planexec_saclocked_curves.png`** — every responsive channel's actual tuning curve (with SEM error bars — see §4.7), stacked visual (top) / planning (middle) / execution (bottom), category-colored.
...
### 6.8 Decoding by area: does the tuning-fraction result generalize?

Direct follow-up to §6.7: does SC's high directional-tuning fraction also show up as better population *decoding*, or is that a separate question? `decodeByArea.m` restricts each session's own decoding population (§4.9's nearest-centroid/LOO method) to just that area's responsive channels in that session, size-matched at 20 channels for a fair cross-area comparison (random subsampling, 20 repeats, when a session has more; all available channels when it has fewer), then pools the resulting accuracy across every session with ≥15 area-X channels. Unlike §6.7, this can't pool raw channels across sessions (trial-level data from different sessions isn't poolable the way per-channel tuning summaries are) — each session contributes one accuracy number per area per epoch, averaged across qualifying sessions.

**The tuning-fraction result does not simply generalize.** SC/CO (42.6% directional in §6.7) decodes only moderately at a matched population size — visual 20.3%, planning 22.6%, execution 24.1% — clearly above chance (12.5%) but not the best of any area. **LMID/MRT/RTMID (49.3% directional, the actual highest tuning fraction in §6.7) and MTHAL (31.2% directional) decode noticeably better** (~28-32% across epochs), consistent with their tuning-fraction lead. But **PCC and F also decode well (~26-31%) despite modest directional-tuning fractions** (PCC: 4.7% directional in §6.7) — a channel population can apparently carry substantial pooled direction information without many of its individual channels reaching the strict single-peaked `directional` threshold, plausibly via the `complex`/non-unimodal responses `classifyTuning.m` was built to detect (§4.7) but which a simple %-directional count doesn't credit. So: %directional is a real but incomplete predictor of decodability — some areas (SC) look better by the tuning-fraction measure than by decoding, and some (PCC, F) the reverse.

Full per-area table and figure (`decode_by_area.png`) saved to `data/figures/pooled/` and `data/decode_by_area.mat`. Same liberal multi-label pooling caveat as §6.7 applies.
