jsonFile = '\\vs03\VS03-VandC-6\Neuropixels_NHP\Data_collection\m032\20240826\recording1\structure.oebin';

% list_open_ephys_binary(jsonFile, 'continuous')
% 
% list_open_ephys_binary(jsonFile, 'events')

settingsFile = '\\vs03\VS03-VandC-6\Neuropixels_NHP\Data_collection\m032\20240826\settings.xml';

savename = '\\vs03\VS03-VandC-6\Neuropixels_NHP\Data_collection\m032\20240826\recording1\tempmua';
...
BTrialDur = [bdCo;bdCo-1;bdCo-2;bdCo-3;bdCo-4]; 
BTrialOn = find(abs(bdata(BTrialDur))==4); %find trial start
...
% words=bits.FullWords;
% 
% BitsTS = bits.Timestamps;
% % BitsTrialDur = 1+find(diff(bits.FullWords)>510); %get trial words
% BitsTrialDur = find(words>521); %get completed trials
% BitsTrialOn = 1+find(diff(BitsTrialDur)>7); %first word bit of each trial, could vary by task
% BitsTrialOnIdx = [BitsTrialDur(1)-4;BitsTrialDur(BitsTrialOn)-4];
% BitsTrialOnTS = BitsTS(BitsTrialOnIdx);
% 
% figure;plot(words)
% hold on
% scatter(BitsTrialOnIdx,words(BitsTrialOnIdx)) %check if onset time is sensible
...
for trl=1:length(Trials)
    ...
    f=find(abs(diode_trl)>4500,1,'first'); %!threshold might vary by task
    if isempty(f)
        dio_onset(trl) = 0;
    else
        dio_onset(trl)= f;
